...
void LevelFour::move()
{
    ...
    for (const std::pair<Coordinate, Coordinate>& p : possible_moves) {
        ...
        std::cout << p.first.column << p.first.row << " " << p.second.column
                  << p.second.row << ": " << mf.value << std::endl;
        // if (mf.check) {
        //     check.insert(p);
        // }
        // if (mf.capture) {
    ...
    // if (!all.empty()) {
    //     return this->execute_best_move(all);
    // }

    // /* next in line is moves that do 2/3 */
    // if (!check_escape.empty()) {
    //     return this->execute_best_move(check_escape);
    // }
    // if (!capture_escape.empty()) {
    //     return this->execute_best_move(capture_escape);
    // }
    // if (!check_capture.empty()) {
    //     return this->execute_best_move(check_capture);
    // }

    // /* finally, do moves that satisfiy 1/3, in this order s*/
    // if (!check.empty()) {
    //     return this->execute_best_move(check);
    // }
    // if (!capture.empty()) {
    //     return this->execute_best_move(capture);
    // }
    // if (!escape.empty()) {
    //     return this->execute_best_move(escape);
    // }

    /* return any moves possible */
    return this->execute_best_move(possible_moves);
}
