...
std::unique_ptr<Player> Chess::create_player(PlayerType type, Color color)
{
    switch (type) {
    case PlayerType::HUMAN:
        return std::make_unique<Human>(this->get_last_game(), color);
    case PlayerType::LEVEL1:
        return std::make_unique<Computer>(this->get_last_game(), color);
...
void Chess::start_game(PlayerType white, PlayerType black)
{
    ...
    // create players
    std::unique_ptr<Player> w = this->create_player(white, Color::WHITE);
    std::unique_ptr<Player> b = this->create_player(black, Color::BLACK);
